...
import { GetAuthenticatedUserSuccessResponse, AuthSuccessResponse, AuthErrorResponse } from '@test-types/auth.types';
...
describe.only('getAuthenticatedUser', () => {
  ...
  it('token válido deve retornar o usuário autenticado', async () => {
    const response: GetAuthenticatedUserSuccessResponse = await getAuthenticatedUser(app, accessToken);
    expectAuthMeSuccess(response, testUser);
  });

  it.each(invalidTokens)('deve lançar erro se token for inválido: %s', async ([_, invalidToken]: [string, any]) => {
    const response: AuthErrorResponse = await getAuthenticatedUser(app, invalidToken);
    expectAuthError(response, 401, ERROR_MESSAGES.INVALID_TOKEN);
  });

  it('token expirado deve retornar erro', async () => {
    const response: AuthErrorResponse = await getAuthenticatedUser(app, expiredToken);
