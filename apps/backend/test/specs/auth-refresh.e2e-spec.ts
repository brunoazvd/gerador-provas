...
import { AuthSuccessResponse, AuthErrorResponse, RefreshAcessTokenSuccessResponse } from '@test-types/auth.types';
...
describe('refresh accessToken', () => {
  ...
  it('deve renovar o accessToken com sucesso', async () => {
    const response: RefreshAcessTokenSuccessResponse = await refreshAccessToken(app, refreshToken);
    expectRefreshAccessTokenSuccess(response, accessToken);
  });

  it('deve retornar aluno se utilizando includeUser flag', async () => {
    const response: RefreshAcessTokenSuccessResponse = await refreshAccessToken(app, refreshToken, true);
    expectRefreshAccessTokenSuccess(response, accessToken, testUser);
  });

  it('deve lançar erro se refreshToken for inválido', async () => {
    const response: AuthErrorResponse = await refreshAccessToken(app, 'invalidToken');
